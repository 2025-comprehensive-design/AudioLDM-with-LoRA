...
filtered_dataset = dataset.filter(lambda example: "hip hop" in example["caption"].lower())
filtered_dataset = filtered_dataset.filter(lambda example: "low quality" not in example["caption"].lower())

# 2. 저장 디렉터리 설정
base_dir = "/home/2020112030/WorkSpace/2025/AudioLDM-with-LoRA/data/dataset/out"
output_audio_dir = os.path.join(base_dir, "wavs")
output_caption_dir = os.path.join(base_dir, "captions")
